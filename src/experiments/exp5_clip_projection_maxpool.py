...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Experiment 5: projection-head fine-tuning with max-image scoring."
    )
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("runs/exp_5_clip_projection_maxpool"))
...
def main() -> None:
    ...
    def score_pairs(pairs: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for pair in tqdm(pairs, desc="Scoring pairs"):
            ...
            row = dict(pair)
            row["similarity"] = float(np.max(sims))
