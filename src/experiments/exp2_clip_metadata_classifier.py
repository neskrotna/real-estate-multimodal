...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Experiment 2: multilingual CLIP features + metadata classifier."
    )
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("runs/exp2_clip_metadata_classifier"))
...
def bool_match(a: Any, b: Any) -> float:
...
def main() -> None:
    ...
    print(f"[INFO] Loading multilingual text model: {args.text-model if False else args.text_model}")
    ...
    def build_features(pairs: list[dict[str, Any]]) -> tuple[np.ndarray, np.ndarray, list[dict[str, Any]]]:
        ...
        for pair in tqdm(pairs, desc="Building features"):
            ...
            text_row = listings_by_id[pair["text_listing_id"]]
            image_row = listings_by_id[pair["image_listing_id"]]

            rooms_abs_diff = abs(safe_float(text_row.get("rooms")) - safe_float(image_row.get("rooms")))
            area_abs_diff = abs(safe_float(text_row.get("area_m2")) - safe_float(image_row.get("area_m2")))
            price_abs_diff = abs(safe_float(text_row.get("price_eur")) - safe_float(image_row.get("price_eur")))

            features = [
                max_sim,
                mean_sim,
                top2_mean,
                float(len(selected_paths)),
                bool_match(text_row.get("city"), image_row.get("city")),
                bool_match(text_row.get("country"), image_row.get("country")),
                bool_match(text_row.get("listing_type"), image_row.get("listing_type")),
                bool_match(text_row.get("property_type"), image_row.get("property_type")),
                bool_match(text_row.get("condition"), image_row.get("condition")),
                bool_match(text_row.get("has_outdoor_space"), image_row.get("has_outdoor_space")),
                rooms_abs_diff,
                float(np.log1p(area_abs_diff)),
                float(np.log1p(price_abs_diff)),
            ]
