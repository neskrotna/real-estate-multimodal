...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Experiment 6: projection-head fine-tuning with weighted image pooling."
    )
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("runs/exp_6_clip_projection_weighted_pooling"))
    ...
    parser.add_argument("--pooling-mode", type=str, default="softmax", choices=["softmax", "top2mean"])
    parser.add_argument("--softmax-alpha", type=float, default=10.0)
...
class ListingEmbeddingDataset(Dataset):
    def __init__(self, listing_ids: list[str], text_embs: dict[str, np.ndarray], pooled_image_embs: dict[str, np.ndarray]):
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        return (
            torch.tensor(self.text_embs[lid], dtype=torch.float32),
            torch.tensor(self.pooled_image_embs[lid], dtype=torch.float32),
        )
...
def main() -> None:
    ...
    per_image_embs: dict[str, dict[str, Any]] = {}
    ...
    for lid in tqdm(all_ids):
        ...
        per_image_embs[lid] = {"paths": image_paths, "embs": embs}

    train_dataset = ListingEmbeddingDataset(split["train"], text_embs, pooled_image_embs)
    val_dataset = ListingEmbeddingDataset(split["val"], text_embs, pooled_image_embs)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for text_x, image_x in train_loader:
            ...
            loss = contrastive_loss(text_z, image_z, args.temperature)
            loss.backward()
            optimizer.step()
    ...
    projected_image_cache: dict[str, dict[str, Any]] = {}
    ...
    def project_images(lid: str) -> dict[str, Any]:
        if lid not in projected_image_cache:
            raw = per_image_embs[lid]
            with torch.no_grad():
                x = torch.tensor(raw["embs"], dtype=torch.float32, device=device)
                z = model.image_head(x)
                z = F.normalize(z, p=2, dim=-1)
                projected_image_cache[lid] = {
                    "paths": raw["paths"],
                    "embs": z.cpu().numpy(),
                }
    ...
    def score_pairs(pairs: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for pair in tqdm(pairs, desc="Scoring pairs"):
            text_vec = project_text(pair["text_listing_id"])
            image_data = project_images(pair["image_listing_id"])
            sims = image_data["embs"] @ text_vec

            if args.pooling_mode == "top2mean":
                sorted_sims = np.sort(sims)[::-1]
                similarity = float(np.mean(sorted_sims[:2])) if len(sorted_sims) >= 2 else float(np.max(sorted_sims))
                weights = None
            else:
                scaled = sims * args.softmax_alpha
                scaled = scaled - np.max(scaled)
                exp_vals = np.exp(scaled)
                weights = exp_vals / np.sum(exp_vals)
                similarity = float(np.sum(weights * sims))

            best_idx = int(np.argmax(sims))
            row = dict(pair)
            row["similarity"] = similarity
            row["best_image_path"] = image_data["paths"][best_idx]
            row["all_image_similarities"] = [float(x) for x in sims.tolist()]
            if weights is not None:
                row["image_weights"] = [float(x) for x in weights.tolist()]
            scored.append(row)
        return scored
